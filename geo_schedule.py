...
def update_database(ip_address, geo_data, alerts):
    ...
    cursor.execute('''INSERT INTO alerts (ip_address, country, city, zip_code, latitude, longitude, alerts_count)
                      VALUES (?, ?, ?, ?, ?, ?, ?)''', 
                      (ip_address, geo_data['country'], geo_data['city'], geo_data['zip'], geo_data['lat'], geo_data['lon'], len(alerts)))
    connection.commit()
    connection.close()
...
def fetch_data():
    ...
    zip_code = geo_data['zip'] or get_zip_code(lat, lon, 'c7cca7e08657441a8b6de18c87f21a2f')  
